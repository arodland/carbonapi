...
func (z zipper) Render(metric string, from, until int32) (expreval.MetricData, error) {
	...
	if err != nil {
		return expreval.MetricData{}, err
	}

	if m := pbresp.Metrics; len(m) == 0 {
		return expreval.MetricData{}, errNoMetrics
	}

	mdata := expreval.MetricData{FetchResponse: *pbresp.Metrics[0]}
